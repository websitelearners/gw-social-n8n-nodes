			...
			try {
				...
				if (resource === 'account') {
					...
					if (operation === 'create') {
						...
						const accountIds = (this.getNodeParameter('accountIds', i) as string).split(',').map(id => id.trim());
						...
						if (updateFields.accountIds) {
							body.account_ids = (updateFields.accountIds as string).split(',').map(id => id.trim());
						}
					} else if (operation === 'delete') {
						requestMethod = 'DELETE';
						const postUuid = this.getNodeParameter('postUuid', i) as string;
